...
fb <- ggplot(two_hosts%>%
             ...
             aes(x = exp(int_host_mass), y = exp(dd_int_host)/15, color = factor(endo_def))) +
  geom_point(aes(shape = parasite_phylum_m),
             size = 0.5, 
             alpha = 0.4) +
  geom_ribbon(data = filter(p, pred == "yes IH"),
              aes(fill = factor(endo_def),
                  x = exp(int_host_mass_c + mean(two_hosts$int_host_mass)),
                  ymin = exp(dd_int_pred.lwr)/15, ymax = exp(dd_int_pred.upr)/15),
              color = NA,
              alpha = 0.2) +
  geom_line(data = filter(p, pred == "yes IH"),
            aes(color = factor(endo_def),
                x = exp(int_host_mass_c + mean(two_hosts$int_host_mass)),
                y = exp(dd_int_pred.fit)/15),
            linetype = "dashed") +
  guides(color = F, fill = F, shape = F) +
  scale_color_brewer(palette = "Set1", direction = -1, labels = c("Ecto next host", "Endo next host")) +
  scale_fill_brewer(palette = "Set1", direction = -1, labels = c("Ecto next host", "Endo next host")) +
  ...
  scale_x_log10(labels = scales::trans_format("log10", scales::math_format(10^.x))) +
  scale_shape_manual(values = c(16,17,15,4)) +
  labs(color = NULL, x = "Intermediate host mass (g)", y = "Larval dev. time\n(days at 20°C)") +
  facet_wrap(~parasite_phylum) +
  theme(panel.grid.minor = element_blank())
fc <- ggplot(two_hosts%>%
...
fd <- ggplot(two_hosts%>%
             ...
             aes(x = exp(int_host_mass), y = exp(dd_def_host)/15, color = factor(endo_def))) +
  geom_point(aes(shape = parasite_phylum_m),
             size = 0.5, 
             alpha = 0.4) +
  geom_ribbon(data = filter(p, pred == "yes IH"),
              aes(fill = factor(endo_def),
                  x = exp(int_host_mass_c + mean(two_hosts$int_host_mass)),
                  ymin = exp(dd_def_pred.lwr)/15, ymax = exp(dd_def_pred.upr)/15),
              color = NA,
              alpha = 0.2) +
  geom_line(data = filter(p, pred == "yes IH"),
            aes(color = factor(endo_def),
                x = exp(int_host_mass_c + mean(two_hosts$int_host_mass)),
                y = exp(dd_def_pred.fit)/15),
            linetype = "dashed") +
  guides(color = F, fill = F, shape = F) +
  scale_color_brewer(palette = "Set1", direction = -1, labels = c("Ecto next host", "Endo next host")) +
  scale_fill_brewer(palette = "Set1", direction = -1, labels = c("Ecto next host", "Endo next host")) +
  ...
  scale_x_log10(labels = scales::trans_format("log10", scales::math_format(10^.x))) +
  scale_shape_manual(values = c(16,17,15,4)) +
  labs(color = NULL, x = "Intermediate host mass (g)", y = "Adult dev. time\n(days at 20°C)") +
  facet_wrap(~parasite_phylum) +
  theme(panel.grid.minor = element_blank())
fe <- ggplot(two_hosts%>%
...
ff <- ggplot(two_hosts%>%
             ...
             aes(x = exp(int_host_mass), y = exp(ag_def_host)/15, color = factor(endo_def))) +
  geom_point(aes(shape = parasite_phylum_m),
             size = 0.5, 
             alpha = 0.4) +
  geom_ribbon(data = filter(p, pred == "yes IH"),
              aes(fill = factor(endo_def),
                  x = exp(int_host_mass_c + mean(two_hosts$int_host_mass)),
                  ymin = exp(ag_def_pred.lwr)/15, ymax = exp(ag_def_pred.upr)/15),
              color = NA,
              alpha = 0.2) +
  geom_line(data = filter(p, pred == "yes IH"),
            aes(color = factor(endo_def),
                x = exp(int_host_mass_c + mean(two_hosts$int_host_mass)),
                y = exp(ag_def_pred.fit)/15),
            linetype = "dashed") +
  guides(color = F, fill = F, shape = F) +
  scale_color_brewer(palette = "Set1", direction = -1, labels = c("Ecto next host", "Endo next host")) +
  scale_fill_brewer(palette = "Set1", direction = -1, labels = c("Ecto next host", "Endo next host")) +
...
f3 <- cowplot::plot_grid(fa + theme(axis.text.x = element_blank(),
                   ...
                   align = "hv", nrow = 3)
f3
# ggsave(f3, filename = "../../figs/fig3_imp.png", width = 9, height = 6)
ggsave(f3, filename = "../../figs/fig3_imp.svg", width = 9, height = 6)
```
...
fb <- ggplot(two_hosts%>%
               mutate(parasite_phylum_m = if_else(missing_dd_ih == "yes", "zz", parasite_phylum)),
             aes(x = exp(def_host_mass), y = exp(dd_int_host)/15, color = factor(endo_def))) +
  geom_point(aes(shape = parasite_phylum_m),
             size = 0.5, 
             alpha = 0.4) +
  geom_ribbon(data = filter(p, pred == "yes DH"),
  ...
  scale_x_log10(labels = scales::trans_format("log10", scales::math_format(10^.x))) +
  scale_shape_manual(values = c(16,17,15,4)) +
  labs(color = NULL, x = "Definitive host mass (g)", y = "Larval dev. time\n(days at 20°C)") +
  facet_wrap(~parasite_phylum) +
  theme(panel.grid.minor = element_blank())
fc <- ggplot(two_hosts%>%
               mutate(parasite_phylum_m = if_else(missing_rg_dh == "yes", "zz", parasite_phylum)),
...
fd <- ggplot(two_hosts%>%
               mutate(parasite_phylum_m = if_else(missing_dd_dh == "yes", "zz", parasite_phylum)),
             aes(x = exp(def_host_mass), y = exp(dd_def_host)/15, color = factor(endo_def))) +
  geom_point(aes(shape = parasite_phylum_m),
             size = 0.5, 
             alpha = 0.4) +
  geom_ribbon(data = filter(p, pred == "yes DH"),
  ...
  scale_x_log10(labels = scales::trans_format("log10", scales::math_format(10^.x))) +
  scale_shape_manual(values = c(16,17,15,4)) +
  labs(color = NULL, x = "Definitive host mass (g)", y = "Adult dev. time\n(days at 20°C)") +
  facet_wrap(~parasite_phylum) +
  theme(panel.grid.minor = element_blank())
fe <- ggplot(two_hosts%>%
               mutate(parasite_phylum_m = if_else(missing_fs == "yes", "zz", parasite_phylum)),
...
ff <- ggplot(two_hosts%>%
               mutate(parasite_phylum_m = if_else(missing_ag == "yes", "zz", parasite_phylum)),
             aes(x = exp(def_host_mass), y = exp(ag_def_host)/15, color = factor(endo_def))) +
  geom_point(aes(shape = parasite_phylum_m),
             size = 0.5, 
             alpha = 0.4) +
  geom_ribbon(data = filter(p, pred == "yes DH"),
...
f4 <- cowplot::plot_grid(fa + theme(axis.text.x = element_blank(),
                   ...
                   align = "hv", nrow = 3)
f4
# ggsave(f4, filename = "../../figs/fig4_imp.png", width = 9, height = 6)
ggsave(f4, filename = "../../figs/fig4_imp.svg", width = 9, height = 6)
```
...
f6 <- cowplot::plot_grid(f6a_1 + theme(axis.title.x = element_blank(),
                         ...
                         f6e_2 + theme(axis.title = element_blank(),
                                       ...
                                       axis.text.y = element_text(size = 7),
                                       plot.margin = unit(c(0, 0, 0, 0), "points")),
                         f6c_1 + theme(axis.text = element_text(size = 7),
                                       plot.margin = unit(c(0, 0, 0, 0), "points")),
                         f6c_2 + theme(axis.title.y = element_blank(),
                                       axis.text.y = element_text(size = 7),
                                       axis.text.x = element_text(size = 9, angle = 45, hjust = 1, vjust = 1),
                                       plot.margin = unit(c(0, 0, 0, 0), "points")),
                         f6f_1 + theme(axis.text = element_text(size = 7),
                                       plot.margin = unit(c(0, 0, 0, 0), "points")),
