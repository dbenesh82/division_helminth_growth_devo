...
wrangle_data <- function(dat){
  dat <- left_join(dat, # add vars indicating which spp had missing host data
                 filter(dat_no_imp, Facultative != "postcyclic", assumed_stage == "no")%>%
                    select(Parasite.species, Stage, host_bm_ni = host_bm, host_tl_ni = host_tl),
  ...
  dat <- dat%>%
    mutate(rg_missing = if_else(is.na(biovolume)|is.na(initial_biov), "yes", "no"),
           ...
           bm_missing = if_else(is.na(host_bm_ni), "yes", "no"))
  ...
  bm_comp <- dat%>%
    ...
    mutate(host_bm_next = lead(host_bm),
           host_bm_ni_next = lead(host_bm_ni),
           rg_next = lead(rg),
           rgr_next = lead(rgr),
           tg_next = lead(tg),
           tgr_next = lead(tgr),
  ...
  bm_comp <- bm_comp%>%
    ...
    select(Parasite.species, parasite_genus, parasite_family, parasite_order, parasite_class, parasite_phylum,
           ...
           Host.no, lcl_max, 
           host_bm, host_bm_next, host_bm_ni, host_bm_ni_next,
  ...
  two_hosts <- filter(bm_comp, lcl_max == 2, parasite_genus != "Echinococcus")%>%
    ...
    rename(int_host_mass = host_bm, def_host_mass = host_bm_next, 
           int_host_mass_ni = host_bm_ni, def_host_mass_ni = host_bm_ni_next, 
           endo_def = endo_next,
  ...
  two_hosts <- filter(two_hosts, !is.na(def_host_mass))
  return(two_hosts)
}
...
```{r}
two_hosts%>%
  summarize(int_host_mass = sum(is.na(int_host_mass_ni)),
            def_host_mass = sum(is.na(def_host_mass_ni)))
```
...
nit <- 800 # number of iterations for models
...
for(i in 1:100){
  ...
  # no cov IH, DH
  chains0[[i]] <- MCMCglmm(cbind(int_host_mass, def_host_mass) ~ trait-1 , 
                ...
                verbose = F)
  # cov IH, DH
  chains1[[i]] <- MCMCglmm(cbind(int_host_mass, def_host_mass) ~ trait-1 , 
  ...
  chains2[[i]] <- MCMCglmm(cbind(int_host_mass, def_host_mass) ~ trait-1 , 
                  random = ~
  ...
  chains2.5[[i]] <- MCMCglmm(cbind(int_host_mass, def_host_mass) ~ trait-1 + trait:parasite_phylum,
                  random = ~
                    idh(trait):parasite_genus +
                    idh(trait):parasite_family +
                    idh(trait):parasite_order +
  ...
  chains3[[i]] <- MCMCglmm(cbind(int_host_mass, def_host_mass) ~ trait-1 ,
                  random = ~
                  ...
                  verbose = F)
  # cov IH, DH and taxonomic covariance
  chains3_ord[[i]] <- MCMCglmm(cbind(int_host_mass, def_host_mass) ~ trait-1 ,
                  random = ~
                    us(trait):parasite_order +
                    idh(trait):parasite_class +
                    idh(trait):parasite_phylum, # covariance among genus, family, order levels
                  ...
                  verbose = F)
  # cov IH, DH and taxonomic covariance
  chains3_fam[[i]] <- MCMCglmm(cbind(int_host_mass, def_host_mass) ~ trait-1 ,
                  random = ~
                    us(trait):parasite_family +
                    us(trait):parasite_order +
                    idh(trait):parasite_class +
  ...
  print(paste('imputation', i, 'finished'))
}
```
...
plot(mcmc.list(mod_comb_dev0, mod_comb_dev1,mod_comb_dev2,mod_comb_dev3), density = F)
```
...
```{r}
cat("DIC, host number (cat);", mean(unlist(lapply(chains3, function(x){x$DIC}))) )
```
...
cat('Delta DIC, int-only vs res covariance:', 
    mean(unlist(lapply(chains0, function(x){x$DIC}))) - mean(unlist(lapply(chains1, function(x){x$DIC}))),
    '(higher is better)')
```
...
cat('Delta DIC, +taxonomy:', 
    mean(unlist(lapply(chains1, function(x){x$DIC}))) - mean(unlist(lapply(chains2, function(x){x$DIC}))),
    '(higher is better)')
```
...
cat('Delta DIC, +taxonomic cov in host mass:', 
    mean(unlist(lapply(chains2, function(x){x$DIC}))) - mean(unlist(lapply(chains3, function(x){x$DIC}))),
    '(higher is better)')
```
...
cat('Delta DIC, +phylum in fixed effects:', 
    mean(unlist(lapply(chains2, function(x){x$DIC}))) - mean(unlist(lapply(chains2.5, function(x){x$DIC}))),
    '(higher is better)')
```

We are mainly interested in the coupling (covariance) between intermediate and definitive host mass.
...
```{r}
summary(
  exp( mod_comb_vcv1[,"traitint_host_mass:traitdef_host_mass.units"]/
         mod_comb_vcv1[,"traitint_host_mass:traitint_host_mass.units"] * log(2)
       ) - 1
)
```
...
summary(
  exp( mod_comb_vcv1[,"traitint_host_mass:traitdef_host_mass.units"]/
         mod_comb_vcv1[,"traitdef_host_mass:traitdef_host_mass.units"] * log(2)
       ) - 1
)
```
...
```
```{r}
ell <- ci_ellipse1(cm = matrix(6*posterior.mode(mod_comb_vcv1[,1:4]), 2,2),
                  mean_x = posterior.mode(mod_comb_sol1[,"traitint_host_mass"]),
                  mean_y = posterior.mode(mod_comb_sol1[,"traitdef_host_mass"]))
```
...
```{r}
ps <- sample(1:length(mod_comb_vcv1[,1]), size = 10)
for(i in ps){
  ell_i <- ci_ellipse1(cm = matrix(6*(mod_comb_vcv1[i,1:4]), 2,2),
                  mean_x = mod_comb_sol1[i,"traitint_host_mass"],
                  mean_y = mod_comb_sol1[i,"traitdef_host_mass"])
  ell_i$p_samp <- i
  if(i == ps[1]){
    ell_many <- ell_i
  } else {
...
```
```{r}
sx <- summary(posterior.cor(mod_comb_vcv1[,1:4]))
cor_anno <- paste0(round(sx$quantiles[2,3], 2),
                   " [", round(sx$quantiles[2,1], 2), " to ",round(sx$quantiles[2,5], 2),"]")
reg <- summarize(two_hosts,
                 x1 = min(int_host_mass),
                 x2 = max(int_host_mass))%>%
  mutate(slope = median(mod_comb_vcv1[,2]/mod_comb_vcv1[,1]))%>%
  mutate(y1 = x1*slope,
         y2 = x2*slope
         )
```
...
```{r}
# variable noting where int host or def host mass missing
two_hosts <- two_hosts%>%
  mutate(missing_bm = if_else(is.na(int_host_mass_ni)|is.na(def_host_mass_ni), "missing", "not missing"))
```


```{r}
f2a <- ggplot(two_hosts%>%
                mutate(missing = if_else(missing_bm == "missing", "zz", parasite_phylum)),
              aes(x = exp(int_host_mass), y = exp(def_host_mass))) +
  geom_vline(xintercept = exp(posterior.mode(mod_comb_sol1[,"traitint_host_mass"])),
             linetype = "dotted") +
  geom_hline(yintercept = exp(posterior.mode(mod_comb_sol1[,"traitdef_host_mass"])),
             linetype = "dotted") +
  ...
  #                            y=exp(y1+posterior.mode(mod_comb_sol1[,"traitdef_host_mass"])), 
  #                            yend=exp(y2+posterior.mode(mod_comb_sol1[,"traitdef_host_mass"])))) +
...
```{r}
ggplot(two_hosts, aes(x = exp(int_host_mass-posterior.mode(mod_comb_sol1[,"traitint_host_mass"])),
                             y = exp(def_host_mass-posterior.mode(mod_comb_sol1[,"traitdef_host_mass"])))) +
  ...
  geom_point(aes(shape = parasite_phylum),
             ...
             size = 1) +
  geom_smooth(method = lm, se = F, color = "black") +
  # geom_segment(data=reg, aes(x=exp(x1-posterior.mode(mod_comb_sol1[,"traitint_host_mass"])),
  #                            xend=exp(x2-posterior.mode(mod_comb_sol1[,"traitint_host_mass"])),
  #                            y=exp(y1-posterior.mode(mod_comb_sol1[,"traitdef_host_mass"])),
  #                            yend=exp(y2-posterior.mode(mod_comb_sol1[,"traitdef_host_mass"])))) +
  scale_y_log10(labels = scales::trans_format("log10", scales::math_format(10^.x))) +
  scale_x_log10(labels = scales::trans_format("log10", scales::math_format(10^.x))) +
  ...
  guides(shape = F) + #guide_legend(override.aes = list(alpha = 1))) +
  annotate("text", label = cor_anno,
           x = exp(max(two_hosts$int_host_mass, na.rm = T) - posterior.mode(mod_comb_sol1[,"traitint_host_mass"])),
           y = exp(max(two_hosts$def_host_mass, na.rm = T) - posterior.mode(mod_comb_sol1[,"traitdef_host_mass"])),
           color = "black", size = 3, hjust = "right")
```


When we add taxonomy to the model, the covariance between intermediate and definitive host mass decreases to around zero.

```{r}
summary(posterior.cor(mod_comb_vcv2[,11:14]))
```
...
```{r}
s1 <- data.frame(
  rbind(summary(posterior.cor(mod_comb_vcv1[,1:4])[,2])$quantiles,
        summary(posterior.cor(mod_comb_vcv2[,11:14])[,2])$quantiles,
        summary(posterior.cor(mod_comb_vcv3[,17:20])[,2])$quantiles
        )
  )
...
```{r}
ell2 <- ci_ellipse1(cm = matrix(6*posterior.mode(mod_comb_vcv2[,11:14]), 2,2),
                  mean_x = posterior.mode(mod_comb_sol2[,"traitint_host_mass"]),
                  mean_y = posterior.mode(mod_comb_sol2[,"traitdef_host_mass"]))
```
```{r}
ggplot(two_hosts, aes(x = int_host_mass, y = def_host_mass)) +
  geom_point() +
...
st_re <- which( grepl(colnames(mod_comb_vcv2), pattern = "traitint_host_mass")) # var comp for int host
...
tax_eff_st <- rbind(
  quantile(mod_comb_vcv2[,"traitint_host_mass:traitint_host_mass.units"], probs = c(0.025, 0.5, 0.975)),
  quantile(mod_comb_vcv2[,"traitint_host_mass.parasite_genus"], probs = c(0.025, 0.5, 0.975)),
  quantile(mod_comb_vcv2[,"traitint_host_mass.parasite_family"], probs = c(0.025, 0.5, 0.975)),
  quantile(mod_comb_vcv2[,"traitint_host_mass.parasite_order"], probs = c(0.025, 0.5, 0.975)),
  quantile(mod_comb_vcv2[,"traitint_host_mass.parasite_class"], probs = c(0.025, 0.5, 0.975)),
  quantile(mod_comb_vcv2[,"traitint_host_mass.parasite_phylum"], probs = c(0.025, 0.5, 0.975)),
  quantile(rowSums(mod_comb_vcv2[,st_re[1:5]]), probs = c(0.025, 0.5, 0.975))
)
...
resi <- which( grepl(colnames(mod_comb_vcv2), pattern = ".units")) # res var-cov
st_re <- which( grepl(colnames(mod_comb_vcv2), pattern = "traitdef_host_mass")) # var comp for ini size
st_re <- st_re[!st_re %in% resi] # just tax var comp
st_re <- c(st_re, 
           which(grepl(colnames(mod_comb_vcv2), 
...
tax_eff_st2 <- rbind(
  quantile(mod_comb_vcv2[,"traitdef_host_mass:traitdef_host_mass.units"], probs = c(0.025, 0.5, 0.975)),
  quantile(mod_comb_vcv2[,"traitdef_host_mass.parasite_genus"], probs = c(0.025, 0.5, 0.975)),
  quantile(mod_comb_vcv2[,"traitdef_host_mass.parasite_family"], probs = c(0.025, 0.5, 0.975)),
  quantile(mod_comb_vcv2[,"traitdef_host_mass.parasite_order"], probs = c(0.025, 0.5, 0.975)),
  quantile(mod_comb_vcv2[,"traitdef_host_mass.parasite_class"], probs = c(0.025, 0.5, 0.975)),
  quantile(mod_comb_vcv2[,"traitdef_host_mass.parasite_phylum"], probs = c(0.025, 0.5, 0.975)),
  quantile(rowSums(mod_comb_vcv2[,st_re[1:5]]), probs = c(0.025, 0.5, 0.975))
)
...
```{r}
resi <- which( grepl(colnames(mod_comb_vcv2), pattern = ".units")) # res var-cov
st_re <- which( grepl(colnames(mod_comb_vcv2), pattern = "traitint_host_mass")) # var comp for ini size
st_re <- st_re[!st_re %in% resi] # just tax var comp
st_re <- c(st_re, 
           which(grepl(colnames(mod_comb_vcv2), 
                       pattern = "traitint_host_mass:traitint_host_mass.units"))) # tax vc + resid vc
...
tax_eff_st <- rbind(
  quantile(mod_comb_vcv2[,"traitint_host_mass.parasite_genus"]/
             rowSums(mod_comb_vcv2[,st_re]), probs = c(0.025, 0.5, 0.975)),
  quantile(mod_comb_vcv2[,"traitint_host_mass.parasite_family"]/
             rowSums(mod_comb_vcv2[,st_re]), probs = c(0.025, 0.5, 0.975)),
  quantile(mod_comb_vcv2[,"traitint_host_mass.parasite_order"]/
             rowSums(mod_comb_vcv2[,st_re]), probs = c(0.025, 0.5, 0.975)),
  quantile(mod_comb_vcv2[,"traitint_host_mass.parasite_class"]/
             rowSums(mod_comb_vcv2[,st_re]), probs = c(0.025, 0.5, 0.975)),
  quantile(mod_comb_vcv2[,"traitint_host_mass.parasite_phylum"]/
             rowSums(mod_comb_vcv2[,st_re]), probs = c(0.025, 0.5, 0.975)),
...
tax_eff_st2 <- rbind(
  quantile(mod_comb_vcv2[,"traitdef_host_mass.parasite_genus"]/
             rowSums(mod_comb_vcv2[,st_re]), probs = c(0.025, 0.5, 0.975)),
  quantile(mod_comb_vcv2[,"traitdef_host_mass.parasite_family"]/
             rowSums(mod_comb_vcv2[,st_re]), probs = c(0.025, 0.5, 0.975)),
  quantile(mod_comb_vcv2[,"traitdef_host_mass.parasite_order"]/
             rowSums(mod_comb_vcv2[,st_re]), probs = c(0.025, 0.5, 0.975)),
  quantile(mod_comb_vcv2[,"traitdef_host_mass.parasite_class"]/
             rowSums(mod_comb_vcv2[,st_re]), probs = c(0.025, 0.5, 0.975)),
  quantile(mod_comb_vcv2[,"traitdef_host_mass.parasite_phylum"]/
             rowSums(mod_comb_vcv2[,st_re]), probs = c(0.025, 0.5, 0.975)),
  quantile(rowSums(mod_comb_vcv2[,st_re[1:5]])/
             rowSums(mod_comb_vcv2[,st_re]), probs = c(0.025, 0.5, 0.975))
)
...
p_f <- lapply(chains3_ord, function(x){predict.MCMCglmm(x, 
                                                         marginal = NULL)})
p_f <- data.frame(p_f)
...
p_g <- lapply(chains3_fam, function(x){predict.MCMCglmm(x, 
                                                         marginal = NULL)})
p_g <- data.frame(p_g)
...
p_c <- lapply(chains3, function(x){predict.MCMCglmm(x, marginal = NULL)})
p_c <- data.frame(p_c)
...
```{r}
# add predicteds and residuals to df
p <- data.frame(hm_int_pred = p_raw[1:l],
                hm_def_pred = p_raw[(l+1):(l*2)],
...
```{r}
res_cn0 <- colnames(mod_comb_vcv3[,grepl(".units",colnames(mod_comb_vcv3))])
res_cn1 <- colnames(mod_comb_vcv3[,grepl(".parasite_genus",colnames(mod_comb_vcv3))])
res_cn2 <- colnames(mod_comb_vcv3fam[,grepl(".parasite_family",colnames(mod_comb_vcv3fam))])
res_cn3 <- colnames(mod_comb_vcv3ord[,grepl(".parasite_order",colnames(mod_comb_vcv3ord))])
res_cn4 <- colnames(mod_comb_vcv1[,grepl(".units",colnames(mod_comb_vcv1))])
sx0 <- summary(posterior.cor(mod_comb_vcv3[,colnames(mod_comb_vcv3) %in% res_cn0]))
sx1 <- summary(posterior.cor(mod_comb_vcv3[,colnames(mod_comb_vcv3) %in% res_cn1]))
sx2 <- summary(posterior.cor(mod_comb_vcv3fam[,colnames(mod_comb_vcv3fam) %in% res_cn2]))
sx3 <- summary(posterior.cor(mod_comb_vcv3ord[,colnames(mod_comb_vcv3ord) %in% res_cn3]))
sx4 <- summary(posterior.cor(mod_comb_vcv1[,colnames(mod_comb_vcv1) %in% res_cn4]))
...
prop_var_per_group <- function(trait){
  ...
  res_var1 <- (mod_comb_vcv3ord[,var1])
  res_var2 <- (mod_comb_vcv3fam[,var1])
  res_var3 <- (mod_comb_vcv3[,var1])
...
Since we report some of the values from the previous two plots in the ms, here are the values at the level of orders:

```{r}
var_df%>%
  filter(level == "order")%>%
  select(-txt, -value, -level_txt)
```
...
tax_cov_plot <- function(trait1, trait2, label1, label2, corner){
  ...
  ell0 <- post_cov_matrix_best(mod_comb_vcv1, trait1, trait2, ".units")
  ell1 <- post_cov_matrices_df(mod_comb_vcv3, trait1, trait2, ".parasite_genus", 10)
  ell2 <- post_cov_matrices_df(mod_comb_vcv3fam, trait1, trait2, ".parasite_family", 10)
  ell3 <- post_cov_matrices_df(mod_comb_vcv3ord, trait1, trait2, ".parasite_order", 10)
  ell4 <- post_cov_matrices_df(mod_comb_vcv3, trait1, trait2, ".units", 10)
  ...
  res_var <- median(mod_comb_vcv1[,var1])
  res_var1 <- median(mod_comb_vcv3ord[,var1])
  res_var2 <- median(mod_comb_vcv3fam[,var1])
  res_var3 <- median(mod_comb_vcv3[,var1])
  ...
  res_var <- median(mod_comb_vcv1[,var2])
  res_var1 <- median(mod_comb_vcv3ord[,var2])
  res_var2 <- median(mod_comb_vcv3fam[,var2])
  res_var3 <- median(mod_comb_vcv3[,var2])
  ...
  regs$slope[which(regs$level=="order")] <- 
    median(mod_comb_vcv3ord[,paste0(corx,".parasite_order")]/mod_comb_vcv3ord[,paste0(var_x,".parasite_order")])
  regs$slope[which(regs$level=="family")] <- 
    median(mod_comb_vcv3fam[,paste0(corx,".parasite_family")]/mod_comb_vcv3fam[,paste0(var_x,".parasite_family")])
  regs$slope[which(regs$level=="genus")] <- 
    median(mod_comb_vcv3[,paste0(corx,".parasite_genus")]/mod_comb_vcv3[,paste0(var_x,".parasite_genus")])
  regs$slope[which(regs$level=="species")] <- 
    median(mod_comb_vcv3[,paste0(corx,".units")]/mod_comb_vcv3[,paste0(var_x,".units")])
...
ggsave(fs1x, filename = "../../figs/figC2_imp.png", width = 6, height = 3)
# ggsave(fs1x, filename = "../../figs/figC2.svg", width = 6, height = 3)
```

What orders and families are responsible for these correlations?

```{r}
mod_comb_sol3ord <- runjags::combine.mcmc(mcmc.list(lapply(chains3_ord, function(x) {x$Sol})))
mod_comb_sol3fam <- runjags::combine.mcmc(mcmc.list(lapply(chains3_fam, function(x) {x$Sol})))
```


```{r}
th_ord_typ <- two_hosts%>%
  group_by(parasite_order)%>%
  summarize(int_host_classes = list(sort(unique(int_host_class))))%>%
  mutate(int_host_classes2 = lapply(int_host_classes, 
                                    FUN = function(x){paste(unlist(x), collapse=', ')}))%>%
  mutate(int_host_classes2 = unlist(int_host_classes2))%>%
  select(-int_host_classes)

sx1 <- mod_comb_sol3ord[,grepl(pattern = "traitint_host_mass.parasite_order", colnames(mod_comb_sol3ord))]
sx <- summary(sx1)
re_fams <- data.frame(parasite_order = row.names(sx$quantiles), 
                      re_lwr = sx$quantiles[,1],
                      re = sx$quantiles[,3],
                      re_upr = sx$quantiles[,5])
re_fams <- re_fams%>%
  mutate(parasite_order = gsub(pattern = "traitint_host_mass.parasite_order.", "", parasite_order))
row.names(re_fams) <- NULL
re_fams <- left_join(re_fams, th_ord_typ)
```
Here are the orders with large intermediate hosts. It includes nematodes with diverse intermediate hosts (Ascaridida, Rhabditida) and cestodes with large amphipod or bivalve intermediate hosts (Amphilinidea, Rhinebothriidea). 
...
Here are the orders with small intermediate hosts. It includes groups that mostly infect zooplankton as intermediate hosts.
...
th_ord_typ <- two_hosts%>%
  group_by(parasite_order)%>%
  summarize(def_host_classes = list(sort(unique(def_host_class))))%>%
  mutate(def_host_classes2 = lapply(def_host_classes, 
                                    FUN = function(x){paste(unlist(x), collapse=', ')}))%>%
  mutate(def_host_classes2 = unlist(def_host_classes2))%>%
  select(-def_host_classes)

sx2 <- mod_comb_sol3ord[,grepl(pattern = "traitdef_host_mass.parasite_order", colnames(mod_comb_sol3ord))]
sx <- summary(sx2)
re_fams <- data.frame(parasite_order = row.names(sx$quantiles), 
                      re_lwr = sx$quantiles[,1],
                      re = sx$quantiles[,3],
                      re_upr = sx$quantiles[,5])
re_fams <- re_fams%>%
  mutate(parasite_order = gsub(pattern = "traitdef_host_mass.parasite_order.", "", parasite_order))
row.names(re_fams) <- NULL
re_fams <- left_join(re_fams, th_ord_typ)
```

Here are orders from large definitive hosts.
...
And here the orders from small definitive hosts (mostly fish).
...
The orders characterized by large intermediate hosts also were characterized by large definitive hosts.

```{r}
ggplot(re_fams, aes(t1, t2)) +
  geom_label(aes(label=parasite_order), alpha = 0.2) +
  labs(x = "Residual IH mass", y = "Residual DH mass") +
  theme(panel.grid.minor = element_blank())
```

We can standardize the random effects and then average them. Positive values indicate groups that have large hosts at both life stages.
...
And here are the groups with small hosts at both stages.
```{r}
re_fams%>%
  arrange(re_upr)%>%
  slice_head(n=10)
...
Host masses varied more among families than orders, so let's now look at the families with large hosts.

```{r}
th_fam_typ <- two_hosts%>%
  group_by(parasite_family)%>%
  summarize(int_host_classes = list(sort(unique(int_host_class))))%>%
  mutate(int_host_classes2 = lapply(int_host_classes, 
                                    FUN = function(x){paste(unlist(x), collapse=', ')}))%>%
...
sx1 <- mod_comb_sol3fam[,grepl(pattern = "traitint_host_mass.parasite_family", colnames(mod_comb_sol3fam))]
sx <- summary(sx1)
re_fams <- data.frame(parasite_family = row.names(sx$quantiles), 
                      re_lwr = sx$quantiles[,1],
                      re = sx$quantiles[,3],
                      re_upr = sx$quantiles[,5])
re_fams <- re_fams%>%
  mutate(parasite_family = gsub(pattern = "traitint_host_mass.parasite_family.", "", parasite_family))
row.names(re_fams) <- NULL
re_fams <- left_join(re_fams, th_fam_typ)
```

Here are the families with large intermediate hosts. It includes taeniid cestodes and numerous nematodes that infect vertebrates as intermediate hosts. 
...
Here are the families with small intermediate hosts. It includes groups that mostly infect zooplankton as intermediate hosts.
...
sx2 <- mod_comb_sol3fam[,grepl(pattern = "traitdef_host_mass.parasite_family", colnames(mod_comb_sol3fam))]
sx <- summary(sx2)
re_fams <- data.frame(parasite_family = row.names(sx$quantiles), 
                      re_lwr = sx$quantiles[,1],
                      re = sx$quantiles[,3],
                      re_upr = sx$quantiles[,5])
re_fams <- re_fams%>%
  mutate(parasite_family = gsub(pattern = "traitdef_host_mass.parasite_family.", "", parasite_family))
row.names(re_fams) <- NULL
re_fams <- left_join(re_fams, th_fam_typ)
```

Here are families with large definitive hosts.
...
And here the families from small definitive hosts, many from fish or rodents.
...
The families characterized by large intermediate hosts often had large definitive hosts.
...
ggplot(re_fams, aes(t1, t2)) +
  geom_point() +
  geom_label(data = re_fams%>%
               filter(parasite_family %in% fams_plot),
             aes(label=parasite_family), alpha = 0.2) +
  labs(x = "Residual IH mass", y = "Residual DH mass") +
  theme(panel.grid.minor = element_blank())
```
...
And here are the families with small hosts at both stages.
```{r}
re_fams%>%
  arrange(re_upr)%>%
  slice_head(n=10)
...
for(i in 1:100){
  ...
  # no cov IH, DH
  chains_lr1[[i]] <- MCMCglmm(endo_def ~ int_host_mass,
                nitt = (250*10)+bi, thin = 250, burnin = bi,
                prior = prior_tax_lr,
                data = as.data.frame(two_hosts_imp),
                family = "categorical",
                pr=F,
                verbose = F)
  chains_lr2[[i]] <- MCMCglmm(endo_def ~ int_host_mass*parasite_phylum,
                nitt = (250*10)+bi, thin = 250, burnin = bi,
                prior = prior_tax_lr,
                data = as.data.frame(two_hosts_imp),
                family = "categorical",
                pr=F,
                verbose = F)
  print(paste('logistic regression, imputation', i, 'finished'))
}
```

```{r}
mod_comb_sol_lr1 <- runjags::combine.mcmc(mcmc.list(lapply(chains_lr1, function(x) {x$Sol})))
mod_comb_sol_lr2 <- runjags::combine.mcmc(mcmc.list(lapply(chains_lr2, function(x) {x$Sol})))
```
...
```{r}
summary(mod_comb_sol_lr1)
```

Here is the predicted percent increase in the odds of infecting an endotherm with a doubling of intermediate host mass.

```{r}
summary( exp(mod_comb_sol_lr1[,"int_host_mass"] * log(2)) - 1) # beta x "log diff corresponding to doubling of size"
```
...
```{r}
mod_comb_dev_lr1 <- runjags::combine.mcmc(mcmc.list(lapply(chains_lr1, name_dev_var)))
mod_comb_dev_lr2 <- runjags::combine.mcmc(mcmc.list(lapply(chains_lr2, name_dev_var)))
```

```{r}
plot(mcmc.list(mod_comb_dev_lr1, mod_comb_dev_lr2), density = F)
```
...
```{r}
summary(mod_comb_sol_lr2)
# plot(mod_comb_sol_lr2)
```
...
```{r}
summary( exp(mod_comb_sol_lr2[,"int_host_mass"] * log(2)) - 1) 
```

for nematodes,

```{r}
summary( exp( (mod_comb_sol_lr2[,"int_host_mass"] + mod_comb_sol_lr2[,"int_host_mass:parasite_phylumNematoda"]) * log(2)) - 1) 
```

and for cestodes.

```{r}
summary( exp( (mod_comb_sol_lr2[,"int_host_mass"] + mod_comb_sol_lr2[,"int_host_mass:parasite_phylumCestoda"]) * log(2)) - 1) 
```

These effect sizes were comparable, but a bit lower when estimated with `glm` instead of `MCMCglmm`.

```{r}
lrx <- glm(endo_def ~ int_host_mass*parasite_phylum, data = two_hosts%>%filter(), family = "binomial")
...
```
```{r}
f2bb <- ggplot(two_hosts,
               aes(y = exp(int_host_mass), 
                   x = parasite_phylum,
                   color = factor(endo_def))) +
  geom_boxplot(outlier.colour = NA) +
  geom_jitter(data = filter(two_hosts, !is.na(int_host_mass_ni)),
              shape = 4,
              position = position_jitterdodge(jitter.width = 0.45),
              alpha = 0.3,
              size = 0.5) +
  geom_jitter(data = filter(two_hosts, !is.na(int_host_mass_ni)),
              aes(shape = parasite_phylum),
              position = position_jitterdodge(jitter.width = 0.45),
              alpha = 0.3,
              size = 0.5) +
...
f2_out <- cowplot::plot_grid(f2a, 
                             ...
                             labels = c("A","", "D"),
                             nrow = 1)
# ggsave(f2_out, filename = "../../figs/fig2_imp.png", width = 7.5, height = 3)
ggsave(f2_out, filename = "../../figs/fig2_imp.svg", width = 7.5, height = 3)
# adjust legend in b, move b/c labels
```


```{r}
# top_row <- cowplot::plot_grid(f2a, f2bb, align = "h", nrow = 1,
#                           labels = c("(a)", "(b)"))
# 
# f2x <- cowplot::plot_grid(top_row, fs1 + labs(title = NULL), 
#                    align = "hv",
#                    ncol = 1,
#                    # rel_widths = c(0.5,0.5,1),
#                    labels = c("", "(c)"))
# ggsave(f2x, filename = "../figs/fig2_new.png", width = 6, height = 6)
# ggsave(f2x, filename = "../figs/fig2_new.svg", width = 6, height = 6)
```

# Conclusions

Parasite species with two-host life cycles infect a wider range of intermediate hosts than definitive hosts (sizes). Also, species that infect larger intermediate hosts tend to infect larger definitive hosts, though the spread of the data indicates that species with small intermediate hosts infect both large and small hosts. This correlation arises because families tend to infect different sized intermediate hosts, suggestive of niche conservatism.

```{r}
save.image(file = "env_decoupling_imp.RData")
```
